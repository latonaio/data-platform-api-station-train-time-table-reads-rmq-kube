...
import (
	dpfm_api_caller "data-platform-api-station-train-time-table-reads-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-station-train-time-table-reads-rmq-kube/config"
	"encoding/json"
	"fmt"
	"time"
...
func getAccepter(input *dpfm_api_input_reader.SDC) []string {
	...
	if accepter[0] == "All" {
		accepter = []string{
			"Header",
			"HeadersByStation",
			"HeadersByRailwayLine",
			"HeadersByRailwayLineWeekdayType",
			"HeadersByRailwayLineWeekdayTypeExpressType",
		}
	}
	return accepter
}
